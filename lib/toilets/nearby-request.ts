 ...
 *
 * `filters` is deliberately not accepted yet: see
 * `docs/adr/0006-nearby-api-contract.md`. Accepting and silently ignoring a
 * field would mislead a caller into thinking it did something.
 */
...
export function parseNearbyRequest(input: unknown): NearbyRequestValidation {
  ...
  const { location, radiusMeters } = result.data;
